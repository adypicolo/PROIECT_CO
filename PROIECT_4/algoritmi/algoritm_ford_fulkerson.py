...
def ruleaza_ford_fulkerson(graf: Graf) -> RezultatRulare:
    ...
    pasi: list[PasAlgoritm] = [
        PasAlgoritm(
            ...
            explicatie=(
                "Pornim cu flux nul pe toate arcele. La fiecare iterație căutăm un drum de creștere în graful rezidual, determinăm capacitatea minimă de pe acel drum și actualizăm fluxul corespunzător."
            ),
            arce_saturate=[],
        )
    ]
    ...
    while True:
        ...
        if rezultat_bfs["drum_crestere"] is None:
            ...
            explicatie_finala = (
                "Nu se mai poate eticheta x10 in graful rezidual, deci algoritmul se oprește.\n"
                f"Tăietura minimă este S = {{{', '.join(multime_s)}}}, T = {{{', '.join(multime_t)}}}.\n"
                f"Verificare: flux maxim = capacitatea tăieturii minime = {formula}"
            )
            pasi.append(
                PasAlgoritm(
                    iteratie=iteratie,
...
def cauta_drum_cu_bfs(
    ...
    etichete_noduri: dict[str, EtichetaNod] = {
        graf.sursa: EtichetaNod(nod=graf.sursa, semn="+", din_nod="-", valoare=None, explicatie="Nodul sursă este etichetat primul.")
    }
    etape_etichetare = [f"Pornim etichetarea din {graf.sursa}."]

    # Sortăm arcele descrescător după capacitate reziduală pentru a explora
    # mai întâi arcele cu capacitate mai mare (comportament similar seminarului).
    arce_sortate = sorted(graf.arce, key=lambda a: a.capacitate, reverse=True)

    while coada:
        nod_curent = coada.popleft()
        for arc in arce_sortate:
            ...
            if arc.sursa == nod_curent:
                ...
                if capacitate_reziduala > 0 and arc.destinatie not in vizitat:
                    ...
                    etichete_noduri[arc.destinatie] = EtichetaNod(
                        ...
                        explicatie=f"{arc.destinatie} primește eticheta + de la {nod_curent}.",
                    )
                    etape_etichetare.append(
                        f"Etichetăm {arc.destinatie} cu [+] din {nod_curent}; disponibil pe {arc.sursa}->{arc.destinatie}: {capacitate_reziduala}."
                    )
            ...
            if arc.destinatie == nod_curent and flux_curent > 0 and arc.sursa not in vizitat:
                ...
                etichete_noduri[arc.sursa] = EtichetaNod(
                    ...
                    din_nod=nod_curent,
                    valoare=flux_curent,
                    explicatie=f"{arc.sursa} primește eticheta - de la {nod_curent}.",
                )
                etape_etichetare.append(
                    f"Etichetăm {arc.sursa} cu [-] din {nod_curent}; putem reduce fluxul pe {arc.sursa}->{arc.destinatie} cu {flux_curent}."
                )
    ...
    if graf.destinatie not in vizitat:
        etape_etichetare.append("Etichetarea se oprește: x10 nu mai poate fi etichetat.")
        return {
            "drum_crestere": None,
    ...
    etape_etichetare.append(f"Am obținut un lanț de creștere: {' -> '.join(drum_crestere)}.")
    return {
        "drum_crestere": drum_crestere,
        "capacitati_reziduale": capacitati_reziduale,
...
def construieste_explicatie_pas(
    ...
    text = [
        f"Iterația {iteratie}",
        "Determinăm un lant de creștere in graful rezidual.",
        *etape_etichetare,
        f"Drumul ales este: {' -> '.join(drum_crestere)}",
        f"Capacități reziduale pe drum: {capacitati_reziduale}",
        f"min({', '.join(str(valoare) for valoare in capacitati_reziduale)}) = {valoare_minima}",
        f"Actualizăm fluxul: {flux_inainte} + {valoare_minima} = {flux_dupa}",
    ]
    if arce_inverse:
        text.append(
            "Pe acest lanț apar si arce inverse, deci reducem fluxul pe: "
            + ", ".join(f"{sursa}->{destinatie}" for sursa, destinatie in arce_inverse)
            + "."
        )
    if arce_saturate:
        text.append(
            "Arce saturate după actualizare: "
